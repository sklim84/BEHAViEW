...
\begin{itemize}
    \item RQ1. \textbf{Behavioral view construction:} 행동 기반 k-NN 보조 view는 거래 그래프 내 의심 계좌 간의 직접 연결 부족을 완화하고 view 수준의 동질성을 회복하는가?
    \item RQ2. \textbf{Subgraph aggregation and encoder stability:} 회복된 view 위에서 서브그래프 풀링은 의심 신호를 추가로 증폭하는가, 그리고 이 효과는 BatchNorm을 포함한 인코더 정규화에 어떻게 의존하는가?
    \item RQ3. \textbf{Self-supervised AML detection under label scarcity:} 라벨 지연 및 cold-start 환경에서 \method 의 자기지도 표현은 지도학습 모델 대비 실질적인 경쟁력을 갖는가?
    \item RQ4. \textbf{Robustness across AML regimes:} 서로 다른 prevalence와 규모를 가진 AML 데이터셋에서도 동일한 메커니즘과 성능 경향이 재현되는가?
\end{itemize}

\subsection{Experimental Setting}

\textbf{Datasets.} 세 AML 데이터셋을 사용한다 (Table~\ref{tab:datasets}). Home and Firm Banking Network(HOFINET)는 한국의 은행 간 이체 결제망에서 발생한 실제 거래를 바탕으로 구축한 약 470만 건의 합성 데이터로, 가계 및 기업 타행 이체 40개월 (2021Q3--2024Q4) 분량의 패턴을 모델링한다. AMLworld~\cite{amlpatterns2024realistic} 는 NeurIPS 2023 공개 합성 벤치마크로 fan-out, scatter-gather, cycle 등 8가지 패턴을 모델링하며, 본 연구는 HI-Small (high illicit ratio, small size) 변형을 사용한다. AMLNet~\cite{huda2025amlnet} 은 high-prevalence regime 검증용 소규모 합성 벤치마크로 AMLworld와 동일 피처 스키마를 공유한다.

\textbf{Account-level labels.} 세 데이터셋 모두 라벨이 거래 (transaction) 단위로 부여된다 (의심 거래 여부). 본 연구는 표준 graph-based AML and fraud detection 관행~\cite{dou2020enhancing,liu2021pick}을 따라 라벨을 계좌 (account) 수준으로 max-aggregate 한다. 즉, 한 번이라도 의심 거래의 출금 또는 입금 측에 관여한 계좌는 의심 (label$=1$), 그렇지 않은 계좌는 정상 (label$=0$) 으로 분류한다. 이 변환 후 의심 계좌 비율은 HOFINET 2.13\%, AMLworld 1.23\%, AMLNet 13.52\% 이다 (Table~\ref{tab:datasets}). 데이터셋 상세와 라벨 도출 의사코드는 부록~\ref{app:datasets} 에 수록한다.
...
\toprule
\textbf{Part} & \textbf{Option} & \textbf{Graph view / BN regime} & \textbf{Contrastive level / encoders} \\
\midrule
\multirow{4}{*}{Setting}
...
\midrule
\multirow{4}{*}{Encoder}
  & \multirow{2}{*}{Per-layer BN} & \multirow{2}{*}{Layer-wise BN} & GBT, DGI+BN, MVGRL+BN, \\
  & & & GRACE+BN, GIN \\
  \cmidrule(l){2-4}
  & Final BN & Output-only BN & BGRL \\
  \cmidrule(l){2-4}
  & BN-less & No BatchNorm & DGI, MVGRL, GRACE, GCA \\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Subgraph Aggregation and Encoder Stability (RQ2)}\label{subsec:rq2}
...
\subsection{Self-Supervised Detection under Label Scarcity (RQ3)}\label{subsec:rq3}
