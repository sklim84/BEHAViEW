...
본 실험의 목적은 \method 의 핵심 주장을 기여 순서에 맞춰 검증하는 데 있다. 먼저 행동 기반 k-NN 보조 view가 저동질성(의심 노드들 간의 연결 부족)을 실제로 완화하는지 확인하고, 다음으로 라벨 지연 및 cold-start 환경에서 라벨 없는 자기지도 학습이 운영적으로 경쟁력 있는지 평가한다. 마지막으로 graph view 구성, 서브그래프 풀링, BatchNorm이 각각 어떤 역할을 하는지를 분해하고, 이 패턴이 다른 AML 데이터셋에서도 재현되는지 검증한다. 이를 위해 우리는 제안한 프레임워크를 아래의 연구 질문들을 통해 평가한다.
\begin{itemize}
    \item RQ1. 행동적 k-NN view가 AML 탐지에 효과적인가? 그리고 왜 효과적인가?
    \item RQ2. 라벨 지연 및 cold-start 환경에서 라벨 없는 자기 지도 학습이 지도 학습 모델에 비해 경쟁력이 있는가?
...
세 데이터셋 모두 라벨이 거래 (transaction) 단위로 부여된다 (의심 거래 여부). 본 연구는 표준 그래프 기반 AML 및 사기 탐지 관행~\cite{dou2020enhancing,liu2021pick}을 따라 라벨을 계좌 (account) 수준으로 최대 집계(max-aggregate) 한다. 즉, 한 번이라도 의심 거래의 출금 또는 입금 측에 관여한 계좌는 의심 (label$=1$), 그렇지 않은 계좌는 정상 (label$=0$) 으로 분류한다. 이 변환 후 의심 계좌 비율은 HOFINET 2.13\%, AMLworld 1.23\%, AMLNet 13.52\% 이다 (Table~\ref{tab:datasets}). 데이터셋 상세와 라벨 도출 의사 코드는 부록~\ref{app:datasets} 에 수록한다.
