...
def main(args):
    ...
    proj_agg = torch.nn.Linear(x_agg.size(1), input_dim)
    proj_cen = torch.nn.Linear(x_cen.size(1), input_dim)
    ...
    contrast_model = BootstrapContrast(loss=loss, mode='L2L').to(device)
    ...
    vis_save_path = f'./_visualize/BGRL_L2L/tsne_{args.model_name}_{args.node_data_name}_{cen_feats}_{args.lr}_{args.input_dim}_{args.hidden_dim}_{args.gconv_nlayers}_{args.loss}.png'
