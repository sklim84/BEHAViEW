...
def train(encoder_model, contrast_model, data, x_struct, optimizer):
    encoder_model.train()
    optimizer.zero_grad()
    z, g, zn = encoder_model(data.x, x_struct, data.edge_index)
...
def test(seed, encoder_model, data, x_struct, vis_save_path, skip_tsne=False):
    encoder_model.eval()
    with torch.no_grad():
        z, _, _ = encoder_model(data.x, x_struct, data.edge_index)
...
def main(args):
    ...
    with tqdm(total=300, desc='(T)') as pbar:
        for epoch in range(1, 301):
            loss = train(encoder_model, contrast_model, data, x_struct.to(device), optimizer)
            pbar.set_postfix({'loss': loss})
            pbar.update()

    os.makedirs('./visualize/DGI_TRN', exist_ok=True)
    struct_feats = "_".join(str(item) for item in args.struct_feats)
    vis_save_path = f'./visualize/DGI_TRN/tsne_{args.model_name}_{args.node_data_name}_{struct_feats}_{args.lr}_{args.input_dim}_{args.hidden_dim}_{args.gconv_nlayers}_{args.loss}.png'
    ...
    result = build_result_dict('DGI_transductive_w_cen', args, test_result, ari_score, sil_score, use_cen=True)
