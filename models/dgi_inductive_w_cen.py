...
def train(encoder_model, contrast_model, data, x_cen, dataloader, optimizer, device):
    ...
    for batch in dataloader:
        ...
        optimizer.zero_grad()
        z, g, zn = encoder_model(batch.x, batch_x_cen, batch.edge_index)
...
def test(seed, encoder_model, data, x_cen, dataloader, device, vis_save_path):
    ...
    with torch.no_grad():
        for batch in dataloader:
            batch = batch.to(device)
            batch_x_cen = x_cen[batch.n_id.cpu()].to(device)
            z, _, _ = encoder_model(batch.x, batch_x_cen, batch.edge_index)
            zs.append(z.cpu())
...
def main(args):
    ...
    with tqdm(total=30, desc='(T)') as pbar:
        for epoch in range(1, 31):
            loss = train(encoder_model, contrast_model, data, x_cen, train_loader, optimizer, device)
            pbar.set_postfix({'loss': loss})
            pbar.update()

    os.makedirs('./visualize/DGI_IND', exist_ok=True)
    cen_feats = "_".join(str(item) for item in args.cen_feats)
    vis_save_path = f'./visualize/DGI_IND/tsne_{args.model_name}_{args.node_data_name}_{cen_feats}_{args.lr}_{args.input_dim}_{args.hidden_dim}_{args.gconv_nlayers}_{args.loss}.png'
    test_result, ari_score, sil_score = test(args.seed, encoder_model, data, x_cen, test_loader, device, vis_save_path)
    print(test_result)
