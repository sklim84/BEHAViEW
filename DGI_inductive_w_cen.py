...
def train(encoder_model, contrast_model, data, x_cen, dataloader, optimizer, device):
    encoder_model.train()
    ...
    for batch_size, node_id, adjs in dataloader:
        ...
        optimizer.zero_grad()
        z, g, zn = encoder_model(data.x[node_id], x_cen, adjs)
...
def test(seed, encoder_model, data, x_cen, dataloader, device, vis_save_path):
    encoder_model.eval()
    zs = []
    for i, (batch_size, node_id, adjs) in enumerate(dataloader):
        # adjs = [adj.to('cuda') for adj in adjs]
        adjs = [adj.to(device) for adj in adjs]
        z, _, _ = encoder_model(data.x[node_id], x_cen, adjs)
        zs.append(z)
...
def main(args):
    ...
    optimizer = Adam(encoder_model.parameters(), lr=args.lr)  # 0.0001

    with tqdm(total=30, desc='(T)') as pbar:
        for epoch in range(1, 31):
            loss = train(encoder_model, contrast_model, data, x_cen.to(device), train_loader, optimizer, device)
            pbar.set_postfix({'loss': loss})
            pbar.update()
    os.makedirs(f'./_visualize/DGI_IND', exist_ok=True)
    cen_feats = "_".join(str(item) for item in args.cen_feats)
    vis_save_path = f'./_visualize/DGI_IND/tsne_{args.model_name}_{args.node_data_name}_{cen_feats}_{args.lr}_{args.input_dim}_{args.hidden_dim}_{args.gconv_nlayers}_{args.loss}.png'
    test_result, ari_score, sil_score = test(seed, encoder_model, data, x_cen.to(device), test_loader, device, vis_save_path)
    print(test_result)
